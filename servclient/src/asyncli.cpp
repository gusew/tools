// from: http://math.msu.su/~vvb/Java/samples/Invert/asyncli.cpp
//
...
int main(int argc, char *argv[]) {
    ...
    char peerHost[10] = "localhost";
    if (argc > 1 && sizeof(argv[1]) < 10) {
      for (unsigned i = 0; i < sizeof(peerHost) && i < sizeof(argv[1]); ++i)
        peerHost[i] = argv[1][i];
    }
...
static void sigHandler(int sigID) {
    printf("The SIGPIPE signal (connection is broken), sigID= %d.\n", sigID);
    finished = 1;
}
